...
@Directive({
  selector: '[numberTwoDigit]'
})
export class NumberTwoDigitDirective {
  @Input() allowDecimals: boolean = true;
  @Input() NoOfDecimalPoints: number = 2;
  ...
  @HostListener('keydown', ['$event']) onKeyDown(e: any) {
    ...
    if (isNumber) {
      const dt = originalValue.split(".");
      if (dt && dt[1] && dt[1].toString().length >= 2) {
        e.preventDefault()
      } else {
        return;
      }
